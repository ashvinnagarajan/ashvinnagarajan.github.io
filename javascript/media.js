var mediaArray = [
    {title: "Design of configuration indifferent compliant building blocks", img_url: "img/precision_eng_media.jpg", date: "2/3/23", description: "Published in Precision Engineering. An approach for designing flexure-based building blocks that achieve certain degrees of freedom which remain unchanged regardless of how they are arranged.", media_url: "https://doi.org/10.1016/j.precisioneng.2023.02.002"},
    {title: "Shell Eco-marathon 2022: On Track - United States", img_url: "img/BruinRacing.jpg", date: "6/1/22", description: "Bruin Racing competed in the 2022 Shell Eco-marathon Americas competition, showcasing our electric super-mileage vehicle on track in the United States.", media_url: "https://www.shellecomarathon.com/about/previous-seasons/2022-programme/on-track/united-states.html"},
    {title: "BruinLabs: UCLA Clubs Offers Free Program to Teach Product-Development Skills", img_url: "img/bruinlabs_media.png", date: "7/21/20", description: "Three UCLA student organizations collaborated to launch BruinLabs, a free eight-week digital course helping nearly 130 students from over 25 universities develop product-building and teamwork skills.", media_url: "https://samueli.ucla.edu/bruinlabs-ucla-clubs-offers-free-program-to-teach-product-development-skills/"},
    {title: "Valedictory Address: Don't succumb to complacency, mediocrity", img_url: "img/dont_succumb_media.jpg", date: "6/28/20", description: "My name is Ashvin Nagarajan, and it is my true honor to address you, my peers, my friends. My honor to have interacted with all of you during the past four years.", media_url: "https://www.newjerseyhills.com/the_progress/opinion/columns/commentary-don-t-succumb-to-complacency-mediocrity/article_38086f47-189d-5b85-a51e-e3a5e29b29fc.html"},
    ...
    {title: "Heterogenous Integration of MEMS Gas Sensor using FOWLP : Personal Environment Monitors", img_url: "img/ectc_media.jpg", date: "1/20/19", description: "In this paper, we have demonstrated the integration of MEMS gas sensors on flexible PDMS substrate using the fan out wafer packaging technique called FlexTrate.", media_url: "ECTC Paper.pdf"},
    {title: "UCLA Idea Hacks 2019", img_url: "img/idea_hacks_media.jpg", date: "1/20/19", description: "Over the weekend of January 18-20, UCLA IEEE and Theta Tau presented the fifth annual Idea Hacks hardware hackathon. The winning team of five first-year students designed and 3D printed a theft-proof bicycle lock called Pop 'n' Lock.", media_url: "https://www.ee.ucla.edu/ucla-idea-hacks-2019/"},
...
function createMediaCard(media) {
    return '<a class="media-card reveal" href="' + media.media_url + '" target="_blank">' +
        '<img class="media-card__img" src="' + media.img_url + '" alt="' + media.title + '" loading="lazy">' +
        '<div class="media-card__body">' +
            '<h3 class="media-card__title">' + media.title + '</h3>' +
            '<p class="media-card__text">' + media.description + '</p>' +
            '<p class="media-card__date">' + media.date + '</p>' +
        '</div>' +
    '</a>';
}
...
for (var i = 0; i < mediaArray.length; i++) {
    mediaGrid.insertAdjacentHTML('beforeend', createMediaCard(mediaArray[i]));
}

// Toggle more media
var toggleMediaBtn = document.getElementById('toggle-media');
var moreMediaGrid = document.getElementById('more-media');
...
toggleMediaBtn.addEventListener('click', function() {
    ...
    if (mediaExpanded) {
        if (moreMediaGrid.children.length === 0) {
            for (var i = 0; i < moreMediaArray.length; i++) {
                moreMediaGrid.insertAdjacentHTML('beforeend', createMediaCard(moreMediaArray[i]));
            }
        }
        moreMediaGrid.classList.remove('is-hidden');
        toggleMediaBtn.textContent = 'Show Fewer';
        observeRevealElements();
    } else {
        moreMediaGrid.classList.add('is-hidden');
        toggleMediaBtn.textContent = 'More Media';
    }
});
